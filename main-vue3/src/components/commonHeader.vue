...
const handleSelect = (e) => {
  switch (e) {
    case 'home':
      ...
      microApp.router.push({ name: 'childHome', path: '/childHome#/' })
      break
    case 'job':
      router.push({ name: 'childJob' })
      break
    case 'enterprise':
      if (router.currentRoute.value.name === 'childEnterprise') {
...
const handleCLickLogo = () => {
  // microApp.router.push({name:'childHome',path:'/main/childAbout'})
  microApp.setData('childEnterprise', { type: Math.random() })
}
</script>
...
<template>
  <div class="common-header">
    <el-menu
      ...
      @select="handleSelect"
    >
      <span class="logo" @click="handleCLickLogo">DOSS直聘</span>
